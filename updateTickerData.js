...
const Ticker = mongoose.model(
    `tickerData${new Date().toISOString().slice(0, 10)}`,
    new Schema({
        ticker: String,
        date: String,
...
const ErrorLog = mongoose.model(
    `errorTickerData${new Date().toISOString().slice(0, 10)}`,
    new Schema({
        ticker: String,
        message: String,
    })
...
async function updateSearchTickerData() {
    ...
    for (let q = 0; q < data.length; q++) {
        ...
        delay = delay + 4500 + 1000 * Math.random()
        if (q % 50 === 0) delay = delay + 15000
        setTimeout(() => {
            getAllData(contender, q)
        }, delay)
    }
...
async function getAllData(input, q) {
    console.log(q)
    let tickerInstance = {}
